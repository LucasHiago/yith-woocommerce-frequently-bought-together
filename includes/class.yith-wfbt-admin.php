...
if ( ! class_exists( 'YITH_WFBT_Admin' ) ) {
	...
	class YITH_WFBT_Admin {
		...
		public function register_panel() {
			...
			$admin_tabs = array(
				'general' => __( 'Settings', 'ywbt' ),
			);

			if ( ! ( defined( 'YITH_WFBT_PREMIUM' ) && YITH_WFBT_PREMIUM ) ) {
				$admin_tabs['premium'] = __( 'Premium Version', 'ywbt' );
			}
		...
		public function plugin_row_meta( $plugin_meta, $plugin_file, $plugin_data, $status ) {
			...
			if ( defined( 'YITH_WFBT_INIT') && YITH_WFBT_INIT == $plugin_file ) {
				$plugin_meta[] = '<a href="' . $this->doc_url . '" target="_blank">' . __( 'Plugin Documentation', 'ywbt' ) . '</a>';
			}
		...
		public function add_bought_together_panel(){
			...
			<div id="yith_wfbt_data_option" class="panel woocommerce_options_panel">
				...
				<div class="options_group">
					...
					<p class="form-field"><label for="yith_wfbt_ids"><?php _e( 'Select products', 'ywbt' ); ?></label>
						<input type="hidden" class="wc-product-search" style="width: 50%;" id="yith_wfbt_ids" name="yith_wfbt_ids" data-placeholder="<?php _e( 'Search for a product&hellip;', 'ywbt' ); ?>" data-multiple="true" data-action="yith_ajax_search_product" data-selected="<?php
